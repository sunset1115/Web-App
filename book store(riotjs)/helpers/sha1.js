 ...
 var mygravity = (function(self){
   self.crypto_hash = (function(){
		...
		(function (A) {
		  function q(a, d, b) {
		    ...
		    if ('HEX' === d) {
		      ...
		      f = g.binLen;
		      e = g.value
		    } else if ('ASCII' === d || 'TEXT' === d) g = v(a, c),
		    f = g.binLen,
		    e = g.value;
		     else if ('B64' === d) g = w(a),
		    f = g.binLen,
		    ...
		    this.getHash = function (a, b, c, d) {
		      ...
		      if ('SHA-1' === a) for (m = 0; m < c; m++) h = s(h, k),
		      k = 160;
		    ...
		    this.getHMAC = function (a, b, d, g, q) {
		      ...
		      m = h.binLen,
		      h = h.value;
		       else if ('ASCII' === b || 'TEXT' === b) h = v(a, c),
		      m = h.binLen,
		      h = h.value;
		       else if ('B64' === b) h = w(a),
		      m = h.binLen,
		    ...
		    for (k = 0; k <= b; k += 1) r[k] = h[k] ^ 909522486,
		    u[k] = h[k] ^ 1549556828;
		    if ('SHA-1' === d) d = s(u.concat(s(r.concat(e), a + f)), a + l);
		     else throw 'Unexpected error in HMAC implementation';
		    return g(d, z(q))
		  }
		}
		function v(a, d) {
		  ...
		  c = 0,
		  g;
		  if ('UTF8' === d) for (g = 0; g < a.length; g += 1) for (f = a.charCodeAt(g), e = [
		  ], 2048 < f ? (e[0] = 224 | (f & 61440) >>> 12, e[1] = 128 | (f & 4032) >>> 6, e[2] = 128 | f & 63)  : 128 < f ? (e[0] = 192 | (f & 1984) >>> 6, e[1] = 128 | f & 63)  : e[0] = f, f = 0; f < e.length; f += 1) b[c >>> 2] |= e[f] << 24 - c % 4 * 8,
		  c += 1;
		   else if ('UTF16' === d) for (g = 0; g < a.length; g += 1) b[c >>> 2] |= a.charCodeAt(g) << 16 - c % 4 * 8,
		  c += 2;
		  return {
		    value: b,
		    binLen: 8 * c
		...
		function w(a) {
		  ...
		  for (e = 0; e < a.length; e += 4) {
		    p = a.substr(e, 4);
		    for (c = g = 0; c <
		    p.length; c += 1) f = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.indexOf(p[c]),
		    g |= f << 18 - 6 * c;
		    for (c = 0; c < p.length - 1; c += 1) d[b >> 2] |= (g >>> 16 - 8 * c & 255) << 24 - b % 4 * 8,
		    b += 1
		  }
		  return {
		    value: d,
		...
		function x(a, d) {
		  ...
		  e,
		  c;
		  for (e = 0; e < f; e += 1) c = a[e >>> 2] >>> 8 * (3 - e % 4),
		...
		function y(a, d) {
		  ...
		  c,
		  g;
		  for (e = 0; e < f; e += 3) for (g = (a[e >>> 2] >>> 8 * (3 - e % 4) & 255) << 16 | (a[e + 1 >>>
		...
		function z(a) {
		  ...
		  try {
		    a.hasOwnProperty('outputUpper') && (d.outputUpper = a.outputUpper),
		    a.hasOwnProperty('b64Pad') && (d.b64Pad = a.b64Pad)
		  } catch (b) {
		  }
		...
		function s(a, d) {
		  ...
		  for (m = 0; m < u; m += 16) {
		    ...
		    g = n[3];
		    p = n[4];
		    for (l = 0; 80 > l; l += 1) b[l] = 16 > l ? a[l + m] : h(b[l - 3] ^ b[l - 8] ^ b[l - 14] ^ b[l - 16], 1),
		    q = 20 > l ? r(h(f, 5), s(e, c, g), p, 1518500249, b[l])  : 40 > l ? r(h(f, 5), t(e, c, g), p, 1859775393, b[l])  : 60 > l ? r(h(f, 5), v(e, c, g), p, 2400959708, b[l])  : r(h(f, 5), t(e, c, g), p, 3395469782, b[l]),
		    p = g,
		    g = c,
		    c = h(e, 30),
		    e = f,
		...
		typeof define.amd ? define(function () {
		  return q
		})  : 'undefined' !== typeof exports ? 'undefined' !== typeof module && module.exports ? module.exports = exports = q : exports = q : A.makeSHA = q
		}) (self);


 		// formats expected ('HEX' or 'B64')
 		self.sha1 = function(text, output_format){
 			var shaObj = new self.makeSHA(text, "TEXT");
			return shaObj.getHash("SHA-1", output_format);
 		};
